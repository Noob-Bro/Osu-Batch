...
- 65 tests passed, including supplied engine/search/GUI tests, first-launch English defaults, the 1.1.1 clear-all-records regression, osu! Songs highlighting in both the download queue and filter results, and the background scan workflow.
...
- Local-song detection reads osu!stable `Songs` folder names and at most one valid `.osu` metadata file per non-numeric song folder to build a shared in-memory `BeatmapSetID` index. A regression test verifies that all difficulties are not reread and that later filter scans reuse the index. It runs in the background and does not modify the library. osu!lazer storage is not scanned; its hash-based file store requires a compatible read-only `client.realm` reader.
