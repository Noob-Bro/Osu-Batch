...
- 62 tests passed, including supplied engine/search/GUI tests, the 1.1.1 clear-all-records regression, osu! Songs library row highlighting, and the background scan workflow.
...
- Local-song detection reads osu!stable `Songs` folder names and `.osu` metadata to match queued `BeatmapSetID` values. It runs in the background and does not modify the library. osu!lazer storage is not scanned.
